...
- The product upload route uses the trained local YOLO/tiled/GNN extraction path
  when model artifacts are present. The legacy `/transcriptions` route still
  uses `heuristic_bootstrap`; do not present that legacy route as trained
  YOLOv8m detector quality.
