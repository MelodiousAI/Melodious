...
Presentation-prep documentation now exists at `docs/PRESENTATION_TECHNICAL_REVIEW.tex`. It is a long-form LaTeX technical review for Ahmad's pre-presentation study: system architecture, detector/GNN/export/app data flow, dataset provenance, every major model/run, metrics tables, metric definitions, the meaning of mAP@0.5 versus mAP@0.5:0.95, the high-level meaning of the 0.95 IoU endpoint, class-coverage limits, stem/dot/rhythm issues, GNN limitations, product-app behavior, deployment story, professor Q&A, and what should and should not be claimed during grading.
...
| M8 - Final Grading Package | Active / packaging | `docs/FINAL_TECHNICAL_REPORT.md`, `docs/FINAL_RUBRIC_EVIDENCE.md`, `docs/BASELINES_AND_GRAPH_COMPARISONS.md`, `docs/RESPONSIBLE_ML.md`; curated `FinalProduct/` package assembly in progress |
...
- Final frozen detector test-set performance has been produced for the selected detector family and inference configuration. Treat `runs/detection/detection_136class_yolov8m_finetune_img1536_maxdet2000_v2_test_final/metrics.json` as a final evaluation artifact, not a tuning target.
...
- The improved M7 detector metric has both selected validation evidence and final held-out test evidence. Keep both split labels explicit in reports.
...
1. Finish curating the `FinalProduct/` package with source, selected model artifacts, selected run evidence, baseline outputs, presentation files, sample inputs, and omission notes.
2. Re-run repository checks after final docs/package assembly: Python tests, metric-claim validation, and frontend build.
3. Keep the tiled result labeled as tiled-validation pilot evidence unless it is separately evaluated on the original full-page validation split.
4. If AWS credentials and account-local values become available, run one public smoke test and save the result under `runs/deploy/`.
5. During presentation prep, lead with the same-candidate-edge graph/non-graph comparison before optional UI extras.
