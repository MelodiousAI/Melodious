...
@lru_cache(maxsize=1)
def load_cached_muscima_data() -> tuple[list[dict], list[dict]]:
    """Load MUSCIMA node/edge data once for on-demand graph generation."""
    return load_muscima_data()
...
def load_graph_data_for_document(document_name: str) -> dict:
    ...
    if not graph_path.exists():
        all_nodes, all_edges = load_cached_muscima_data()
...
def build_document_training_export(document_name: str) -> dict:
    ...
    image_path = ensure_document_image(document_name, document_nodes=document_graph["nodes"])
