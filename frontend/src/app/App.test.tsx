...
const configResponse = {
  ...
  upload_status_message: 'Upload a score image.',
  feature_flags: {
...
describe('App', () => {
  ...
  it('uploads a score image and shows generated downloads', async () => {
    const uploadResponse = {
      ...
      sample: {
        ...
        document_name: 'upload-123',
        preview_image_url: '/product/uploads/upload-123/image',
        description: 'Uploaded score.',
        tags: ['Uploaded'],
      },
      score_preview: {
        image_url: '/product/uploads/upload-123/image',
        alt_text: 'Preview image for simple score',
      },
      downloads: {
        musicxml: {
          ready: true,
          url: '/product/uploads/upload-123/downloads/musicxml',
          file_name: 'upload-123.musicxml',
          content_type: 'application/vnd.recordare.musicxml+xml',
        },
        midi: {
          ready: true,
          url: '/product/uploads/upload-123/downloads/midi',
          file_name: 'upload-123.mid',
          content_type: 'audio/midi',
        },
      },
      notices: ['OpenCV fallback detector used.'],
    }
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input)

      if (url.endsWith('/product/transcribe-image') && init?.method === 'POST') {
    ...
    const file = new File(['fake-png'], 'simple-score.png', { type: 'image/png' })
    fireEvent.change(input, { target: { files: [file] } })

    expect(screen.getByText(/Ready to transcribe/i)).toBeInTheDocument()
    screen.getByRole('button', { name: /Transcribe score/i }).click()

    await waitFor(() => {
      expect(screen.getByText(/Transcription ready/i)).toBeInTheDocument()
    })

    expect(screen.getByText(/Download MusicXML/i)).toBeInTheDocument()
    expect(screen.getByText(/Download MIDI/i)).toBeInTheDocument()
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringMatching(/\/product\/transcribe-image$/),
