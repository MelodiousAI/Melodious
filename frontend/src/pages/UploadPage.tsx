...
import { resolveAssetUrl, transcribeImage } from '../lib/api'
import type { ProductTranscriptionResult } from '../types/product'

type InputMode = 'upload' | 'camera'
type UploadState = 'idle' | 'drag-active' | 'file-selected' | 'processing' | 'success' | 'unsupported' | 'error'
...
export function UploadPage() {
  ...
  const [transcriptionResult, setTranscriptionResult] = useState<ProductTranscriptionResult | null>(null)

  const isSupported = (name: string) => {
    const extension = name.split('.').pop()?.toUpperCase() ?? ''
    return supportedFormats.filter((format) => format !== 'PDF').includes(extension)
  }
  ...
  const handleFile = (file: File) => {
    ...
    if (isSupported(file.name)) {
      ...
      setErrorMessage(`"${file.name}" is not a supported format. Please use PNG, JPG, JPEG, or TIFF.`)
    }
  }
  ...
  const handleTranscribeUpload = async () => {
    ...
    try {
      const result = await transcribeImage(selectedFile)
              ...
              {uploadState === 'idle' || uploadState === 'drag-active' ? (
                <>
                  ...
                  <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-primary px-6 py-2.5 font-sans text-sm font-semibold text-primary-foreground shadow-sm transition-opacity hover:opacity-90">
                    ...
                    Choose file
                    <input type="file" className="hidden" accept=".png,.jpg,.jpeg,.tiff" onChange={handleInputChange} />
                  </label>
                  <p className="mt-4 font-sans text-xs text-muted-foreground">
                    Supported: PNG, JPG, JPEG, TIFF
                  </p>
                </>
              ) : uploadState === 'file-selected' || uploadState === 'processing' || uploadState === 'success' ? (
                <>
                  <CheckCircle className="mx-auto mb-4 h-10 w-10 text-[hsl(var(--success))]" />
                  <p className="mb-1 font-display text-lg font-semibold text-foreground">
                    {uploadState === 'success' ? 'Transcription ready' : 'File selected'}
                  </p>
                  ...
                  {uploadState === 'success' && transcriptionResult ? (
                    <div className="mx-auto max-w-md space-y-4 rounded-lg border border-border bg-card p-5 text-left">
                      <div>
                        <p className="font-sans text-sm font-semibold text-foreground">
                          {transcriptionResult.confidence_indicator.label}
                        </p>
                        <p className="mt-1 font-body text-xs leading-relaxed text-muted-foreground">
                          {transcriptionResult.transcription_summary.note_count} notes,{' '}
                          {transcriptionResult.transcription_summary.clef_count} clefs, and{' '}
                          {transcriptionResult.transcription_summary.rest_count} rests were exported.
                        </p>
                      </div>
                      {transcriptionResult.notices.length > 0 ? (
                        <div className="rounded-md bg-accent p-3 font-body text-xs leading-relaxed text-muted-foreground">
                          {transcriptionResult.notices[0]}
                        </div>
                      ) : null}
                      <div className="flex flex-col gap-2 sm:flex-row">
                        <a
                          href={resolveAssetUrl(transcriptionResult.downloads.musicxml.url)}
                          className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 font-sans text-sm font-semibold text-primary-foreground shadow-sm transition-opacity hover:opacity-90"
                        >
                          <FileText className="h-4 w-4" />
                          Download MusicXML
                        </a>
                        <a
                          href={resolveAssetUrl(transcriptionResult.downloads.midi.url)}
                          className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg border border-border px-4 py-2.5 font-sans text-sm font-medium text-foreground transition-colors hover:bg-accent"
                        >
                          <FileText className="h-4 w-4" />
                          Download MIDI
                        </a>
                    ...
                    <div className="inline-flex items-center gap-3 rounded-lg border border-border bg-card px-5 py-3">
                      ...
                      <div className="text-left">
                        <p className="font-sans text-sm font-medium text-foreground">
                          {uploadState === 'processing' ? 'Transcribing your score...' : 'Ready to transcribe'}
                        </p>
                        <p className="font-body text-xs text-muted-foreground">
                          {uploadState === 'processing'
                            ? 'Melodious is detecting symbols and generating MusicXML/MIDI exports.'
                            : 'Run the upload through the detector/export pipeline.'}
                        </p>
                      </div>
                    </div>
                  )}
                  {uploadState !== 'success' ? (
                    <button
                      ...
                      disabled={uploadState === 'processing'}
                      className="mt-5 inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-2.5 font-sans text-sm font-semibold text-primary-foreground shadow-sm transition-opacity hover:opacity-90 disabled:cursor-wait disabled:opacity-60"
                    >
                      {uploadState === 'processing' ? 'Processing...' : 'Transcribe score'}
                    </button>
                  ) : null}
                </>
              ) : (
