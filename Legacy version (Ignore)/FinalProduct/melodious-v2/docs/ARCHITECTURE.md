...
- Purpose: local testing and product-route execution of clean sheet image note
  extraction. The `/product/transcribe-image` route uses this path; the older
  `/transcriptions` compatibility route remains `heuristic_bootstrap`.
