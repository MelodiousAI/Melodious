...
- **Edge/mobile bonus:** No unless a real edge/mobile run is separately demonstrated
...
- **RM2 (bias/fairness):** `MODEL_CARD.md` (Western-notation bias, excluded notation systems, printed-vs-handwritten subgroup disparity check, OOD warning).
...
- **EN3 (reproducible run path):** `README.md` (main commands/tests + deterministic runbook section + verified smoke section).
- **EN4 (functional demo/UI):** `frontend/` app and `src/ui/streamlit_app.py`; product routes in `src/api/product_routes.py`.
- **EN5 (stable service setup):** `README.md` verified API smoke (`/health`, `/product/config`, `/product/transcribe`), `docker-compose.yml`, API health/assemble/midi endpoints.
...
- **PR4 preparation:** `presentation_script.md` (5-minute script + demo sequence + prepared Q&A answers).
