...
For demo and submission review, start with `START_HERE_FOR_DEMO.md`. That file gives the shortest rubric-oriented run path and lists the known grading risks.
...
This clean package intentionally omits the raw large datasets. It includes curated sample data and processed artifacts needed for smoke checks and rubric evidence:

- MUSCIMA XML demo pages: `MUSICMA_Sample/`
- Detector/reference payloads: `sample_detections/`
- Generated MUSCIMA preview images: `data/processed/generated_muscima_images/`
- Five-page graph and training exports: `data/processed/graphs/` and `data/processed/training_exports/`

Full training regeneration requires the original raw datasets described in `documentation.md`; normal demo verification does not.
...
```powershell
# Terminal 1, from FilesForDemo
..\.venv\Scripts\python.exe -m uvicorn src.api.app:app --host 0.0.0.0 --port 8000

# Terminal 2, from FilesForDemo\frontend
$env:VITE_API_BASE_URL="http://YOUR_LAPTOP_IP:8000"
npm run dev -- --host 0.0.0.0
```

Open `http://YOUR_LAPTOP_IP:5173/upload` on a phone on the same Wi-Fi network. The upload/camera path runs YOLOv8 symbol detection with `outputs/yolov8_extended/train/weights/best.pt`; MusicXML/MIDI export for arbitrary uploads is not claimed.
...
from the clean demo package. The local verification reused the parent checkout's
