...
Before starting any new GNN retraining, the current product-path state was saved as Git tag `pre-gnn-retrain-20260606`. The product-code checkpoint commit is `c253cb2`, and the tag is maintained as the rollback target for the documented pre-retrain state. The default runtime GNN checkpoint remains `..\outputs\gnn_checkpoint.pt` with SHA256 `065a6881645c080605eb58742cc3f004322b6fca3e712f8bb2953ddb7f038eab`; retraining must write to a separate V2 run directory until it beats the saved checkpoint on graph metrics and local demo MusicXML. The first retraining run is active under `runs/graph/graph_legacy_gnn_muscima_retrain_20260606/`, using `..\data\muscima-pp\v2.0\data\annotations`. It resumes from the default checkpoint, runs on CUDA, writes its command to `train_command.ps1`, writes the parent PID to `train.pid`, and keeps logs in `train_stdout.log` and `train_stderr.log`. A failed first launch caused by Windows `cp1252` redirected-output encoding was preserved as failed logs plus `gnn_checkpoint_failed_epoch1.pt`; the active relaunch sets `PYTHONUTF8=1` and `PYTHONIOENCODING=utf-8`.
