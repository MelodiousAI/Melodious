...
Before starting any new GNN retraining, the current product-path state was saved as Git tag `pre-gnn-retrain-20260606` at commit `c253cb2`. The default runtime GNN checkpoint remains `..\outputs\gnn_checkpoint.pt` with SHA256 `065a6881645c080605eb58742cc3f004322b6fca3e712f8bb2953ddb7f038eab`; retraining must write to a separate V2 run directory until it beats the saved checkpoint on graph metrics and local demo MusicXML. The first planned retraining run directory is `runs/graph/graph_legacy_gnn_muscima_retrain_20260606/`, using `..\data\muscima-pp\v2.0\data\annotations`.
