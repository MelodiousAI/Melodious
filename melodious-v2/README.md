...
12. The current tiled smoke dataset is `runs/data/deepscores_136_yolo_tiled_stem_smoke_v1/`. It retained 4361 stem labels and raised projected median stem width from roughly `0.78` model pixels on whole pages to `2.666645333333387` pixels at tiled `target_imgsz=1024`.
13. No tiled detector metric has been claimed yet. A tiled training run must write its own `runs/**/metrics.json` before it can be used as model-performance evidence.
