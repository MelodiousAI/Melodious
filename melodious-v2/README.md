...
13. The active no-copy tiled pilot is `detection_136class_yolov8m_tiled_stem_pilot_img1024_v1`, using `runs/data/deepscores_136_yolo_tiled_stem_pilot_v1/` with 12000 train, 2500 validation, and 2500 test tile paths. No tiled detector metric has been claimed yet; the pilot must write its own `runs/**/metrics.json` before it can be used as model-performance evidence.
