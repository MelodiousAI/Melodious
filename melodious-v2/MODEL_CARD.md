...
- The saved full-page demo checkpoint is the default for local note extraction because direct tiled-pilot inference on full pages is a different input distribution. Use `--checkpoint` only when intentionally comparing a different checkpoint and document the output directory separately from detector metrics.
