...
- The completed v2 fine-tune improved the headline validation metrics, but `stem` remains `0.0` mAP and `ledgerLine` is only `0.01106603897644983`. Rhythm extraction on uploaded sheet demos therefore remains limited by missing stem detections.
- The current engineering fix for `stem` is tiled training, not another blind whole-page epoch increase. The full tiled data shows stems become visible small objects and the pilot is running, but final model evidence still requires a completed tiled run with `runs/**/metrics.json`.
