...
- Completed 1472 fine-tune AP metrics: `mAP@0.5:0.95 = 0.6777474953487629` and `mAP@0.5 = 0.8226206920791271`.
...
- The completed follow-up fine-tune `detection_136class_yolov8m_finetune_img1536_maxdet2000_v2` is recorded in `runs/detection/detection_136class_yolov8m_finetune_img1536_maxdet2000_v2/metrics.json` after corrected finalization at image size 1536 with `max_det=2000`.
...
- A stem-focused tiled dataset path now exists for the main rhythm blocker. The full tiled dataset `runs/data/deepscores_136_yolo_tiled_stem_v1/` contains 88137 train tiles, 10709 validation tiles, 26019 test tiles, and 747473 retained stem labels across all splits. A sampled no-copy pilot dataset `runs/data/deepscores_136_yolo_tiled_stem_pilot_v1/` points to 12000 train, 2500 validation, and 2500 test tiles from the full tiled set.
- The completed tiled pilot run is `detection_136class_yolov8m_tiled_stem_pilot_img1024_v1`, recorded in `runs/detection/detection_136class_yolov8m_tiled_stem_pilot_img1024_v1/metrics.json`.
...
- The completed tiled pilot was saved to `artifacts/manual_checkpoints/detection_136class_yolov8m_tiled_stem_pilot_img1024_v1/final_epoch12_completed_2026-06-04_225710/`; copied `best.pt` and `last.pt` both loaded successfully as 136-class detect checkpoints.
- The local note-extraction CLI defaults to the saved full-page demo checkpoint at `artifacts/models/note_extraction_default_fullpage/best.pt` when that generated artifact exists. It is copied from `detection_136class_yolov8m_finetune_img1472_maxdet2000_v1` and used for full-page demo transcription, not as a new metric run.
- The local note-extraction CLI also defaults to the saved tiled stem pilot checkpoint at `artifacts/models/note_extraction_tiled_stem_pilot/best.pt` when that generated artifact exists. This checkpoint runs on overlapping source-pixel tiles to supply thin-symbol detections for stems, beams, flags, and explicit accidentals.
- The local note-extraction CLI uses `..\outputs\gnn_checkpoint.pt` when available and now consumes GNN `stem_notehead` and `beam_notegroup` relationships during rhythm inference before MusicXML export.
- Fur Elise local demo evidence after tiled thin-symbol inference, GNN rhythm integration, line/space pitch quantization, and step-matched accidental attachment is stored under `runs/demo/fur_elise_pitch_accidental_fix_tiled_gnn_20260605/`. Compared with the staff-fixed full-page run, it keeps 9 staff systems and 256 note events, keeps dotted notes at 3, improves stem-confirmed notes from 0 to 171, and extracts the opening phrase as `E5`, `D#5`, `E5`, `D#5`, `E5`, `B4`, `D5`, `C5`, `A4`. This is a local demo transcription result, not an official detector metric claim.
...
- The completed v2 fine-tune improved the headline full-page validation metrics, but `stem` remains `0.0` mAP and `ledgerLine` is only `0.01106603897644983` on original full-page validation. Rhythm extraction on uploaded sheet demos therefore remains limited unless a tiled/full-page inference strategy is intentionally wired in and tested.
- The completed tiled pilot shows that `stem` can be learned when symbols are visible at tile scale, but those metrics are tiled-validation pilot evidence. They must not be reported as original full-page validation performance unless the checkpoint is separately evaluated on the original full-page validation split.
- The saved full-page demo checkpoint remains the default notehead/full-page context checkpoint for local note extraction. The tiled stem pilot is used as a second tiled thin-symbol pass, not as a replacement full-page checkpoint.
- The local note-extraction demo disables CV augmentation-dot guessing for YOLO-backed runs by default. Tiled augmentation-dot detections are also disabled by default after the Fur Elise test showed they can create many false dotted notes. This reduces false dotted notes but does not solve all rhythm errors.
- The tiled+GNN local extraction path improves stem evidence on the tested Fur Elise page, but rhythm can still be musically wrong because beam counts, voices, barlines, measures, ties, and measure-scoped accidentals remain heuristic or incomplete.
