...
def get_uploaded_score_image(upload_id: str):
    """Return the stored preview image for one uploaded transcription."""
    image_path = get_uploaded_image_path(upload_id)
    return FileResponse(image_path)


@router.get("/uploads/{upload_id}/downloads/{output_format}")
def download_uploaded_asset(upload_id: str, output_format: Literal["musicxml", "midi"]):
    """Return one export asset generated from an uploaded image."""
    content, content_type, file_name = build_uploaded_download_asset(upload_id, output_format)
    headers = {"Content-Disposition": f'attachment; filename="{file_name}"'}
    return Response(content=content, media_type=content_type, headers=headers)
...
@router.post("/transcribe-image", response_model=ProductTranscriptionResponse)
async def transcribe_product_image(file: UploadFile = File(...)):
    """Run the public image-upload transcription flow."""
    return transcribe_uploaded_image(file.filename or "uploaded-score.png", await file.read())
