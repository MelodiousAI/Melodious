...
class TestProductRoutes(unittest.TestCase):
    ...
    def test_product_config_exposes_week_5_flags(self):
        ...
        self.assertEqual(body["active_experience"], "upload_enabled")
        self.assertTrue(body["feature_flags"]["image_upload_enabled"])
    ...
    def test_product_transcribe_image_accepts_upload_and_returns_downloads(self):
        image = Image.new("RGB", (360, 160), "white")
        draw = ImageDraw.Draw(image)

        for y in [60, 68, 76, 84, 92]:
            draw.line((20, y, 340, y), fill="black", width=1)

        for x, y in [(120, 76), (165, 68), (210, 84)]:
            draw.ellipse((x - 5, y - 4, x + 5, y + 4), fill="black")
        ...
        response = self.client.post(
            ...
            files={"file": ("simple-score.png", buffer, "image/png")},
        )

        self.assertEqual(response.status_code, 200)
        body = response.json()
        self.assertTrue(body["sample"]["id"].startswith("upload-"))
        self.assertIn("/product/uploads/", body["score_preview"]["image_url"])
        self.assertGreater(body["transcription_summary"]["detection_count"], 0)
        self.assertTrue(body["downloads"]["musicxml"]["ready"])
        self.assertTrue(body["downloads"]["midi"]["ready"])
        self.assertIn("OpenCV fallback", " ".join(body["notices"]))

        musicxml_response = self.client.get(body["downloads"]["musicxml"]["url"])
        self.assertEqual(musicxml_response.status_code, 200)
        self.assertIn(b"<score-partwise", musicxml_response.content)
