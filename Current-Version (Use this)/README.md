...
Melodious V2 is a clean rebuild of the original OMR project. It targets an applied/deployment project: full-taxonomy detection, measured graph assembly, real upload-to-export service behavior, and cloud deployment.
...
The current product upload route, `POST /product/transcribe-image`, calls the
same trained local extraction path documented in `docs/NOTE_EXTRACTION_DEMO.md`.
The older compatibility route, `POST /transcriptions`, remains a bootstrap
sample/heuristic path and should not be used for trained uploaded-image demo
claims.
...
The latest Fur Elise local demo run with tiled stem inference, GNN rhythm
evidence, line/space pitch quantization, and step-matched accidental attachment
is `runs/demo/fur_elise_pitch_accidental_fix_tiled_gnn_20260605/`. Compared
with the staff-fixed full-page run, it keeps 9 staff systems and 256 note events,
keeps dotted notes at 3, improves stem-confirmed notes from 0 to 171, and
extracts the opening phrase as `E5`, `D#5`, `E5`, `D#5`, `E5`, `B4`, `D5`,
`C5`, `A4`. It writes MusicXML at
